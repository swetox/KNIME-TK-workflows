...
# Work week: Xh/d, Yd/w for Z days
# Source: Ernsgaard, 2014
# R script: by Signe Klinting, 2017

####################### PBPK model #################################

pbpkWW <- function(Time, Concentration, parameters){
  with(as.list(c(Concentration, parameters)), {
    WW		= ifelse(Time %% (24*60*7) >= (24*60*WDperWeek), 0, 1)
    DW		= ifelse(Time %% (24*60) >= (WHperDay*60), 0, 1)
    AW 		= DW*WW
...
# Time course:
days 	= knime.in$"SimLength"
start 	= 0			
stop 	= days*24*60
times 	= seq(start, stop, time_step)

# Work hours and days
WHperDay = knime.in$"WHperDay"
WDperWeek= knime.in$"WDperWeek"
...
############ Solving #######################################
solution <- ode(y = yini, times = times, func = pbpkWW, parms = parameters, method = method)
